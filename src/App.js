...
function App() {
  ...
  return (
    <>
      <DropdownButton id="dropdown-basic-button " variant="danger" title={!chart[0] ? "Select a Country" : chart[0].first}
>
        {data && data.map((prop, i) => {
          return <Dropdown.Item  onClick={firstCountry} key={i}>{prop.State}</Dropdown.Item>;
        })}
      </DropdownButton>
      <DropdownButton id="dropdown-basic-button" title={!chart[0] || !chart[0].second ? "Select a Country" : chart[0].second}>
        {data && data.map((prop, i) => {
      ...
      {chart.length > 0 && <ResponsiveContainer width="95%" height={400}>
        ...
        <Line type="monotone" dataKey={chart[0].first} stroke="#8884d8" />
          <Line type="monotone" dataKey={chart[0].second} stroke="#8884d8" />
